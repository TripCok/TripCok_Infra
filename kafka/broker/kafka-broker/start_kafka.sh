...
BROKER_PROPERTIES="/opt/kafka/config/broker.properties"  # Kafka 브로커 설정 파일 경로
...
if [ "$CLUSTER_ID" != "$STORED_CLUSTER_ID" ]; then
    ...
    rm -f /tmp/kraft-broker-logs/meta.properties

    # meta.properties 내용 출력
    cat /tmp/kraft-broker-logs/meta.properties

    # Kafka를 포맷
    $KAFKA_HOME/bin/kafka-storage.sh format -t "$CLUSTER_ID" -c "$BROKER_PROPERTIES"
